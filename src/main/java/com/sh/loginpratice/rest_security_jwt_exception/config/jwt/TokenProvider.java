package com.sh.loginpratice.rest_security_jwt_exception.config.jwt;
...
/**
 * JWT에서 가장 중요한 Token을 생성하고 Token을 인증하고 Token으로 부터 Authentication 객체를 가져오도록 도와주는 TokenProvider 객체를 생성합니다
 */
@Slf4j
@Getter
@Component
public class TokenProvider {
    ...
    public TokenProvider(@Value("${jwt.secret}") String secretKey ,
                         @Value("${jwt.expireMin}") long accessTime,
                         @Value("${jwt.refreshExpireMin}") long refreshTime) {
     ...
     * CustomEmailPasswordAuthToken은 UsernamePasswordAuthenticationToken 을 커스터마이징한 클래스이다.
     * @param accessToken
     * @return
     * @throws BizException
     */
